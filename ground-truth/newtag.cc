...
double MINV[]={1,1.1,1.5};
double THRESH[]={3,5,10,15};
...
map <celltype, double> multiplier;

double allowed[] = {5,10,50,100};
...
struct record {
  int n;
  unsigned int stime;
  map<celltype, map<int, ccell>> records;
};
...
{
  ofstream out;
  out.open("ntags.txt", std::ios_base::out);
	  ...
	  for (auto dit = iit->second.data.begin(); dit != iit->second.data.end(); dit++)
	    {
	      ...
	      for (int i = 0; i< 48; i++)
		{
		  if (dit->second.tag[i] > 0)
		    out<<"A"<<dit->second.tag[i]<<" ";
		  else
		    out<<"0 ";
		}
	      out<<",";
...
{
    for (int i = 0; i < 3; i++)// MINV
    for (int j = 0; j < 4; j++) // THRESH
      for (int k = 0; k < 4; k++) // ALLOWED
	{
	  int index = i*4*4+j*4+k;
		...
		if(metrics[ip][t].n > 1)
		  {
		    double std = sqrt(metrics[ip][t].records[ct][index].ss/(metrics[ip][t].n-1));
		    if (std < 1)
		      std = 1;
		    double tmp = metrics[ip][t].records[ct][index].cusum + (data - metrics[ip][t].records[ct][index].mean)/std;
		    if (tmp > 0)
		      {
			metrics[ip][t].records[ct][index].cusum = tmp;
			if (metrics[ip][t].records[ct][index].cusum > 2*THRESH[j])
			  metrics[ip][t].records[ct][index].cusum = 2*THRESH[j];
		      }
		    else
		      {
			metrics[ip][t].records[ct][index].cusum = 0;
		      }
		  }
	      }
	  }
...
void update_means(unsigned int ip, enum type t, struct bcell value, unsigned int time)  
{
  for (int i = 0; i < 3; i++)// MINV
      ...
      for (int k = 0; k < 4; k++) // ALLOWED
	{
		...
		if (metrics[ip][t].records[ct][index].cusum <= THRESH[j] || metrics[ip][t].n < MINS || data <= MINV[i]*metrics[ip][t].records[ct][index].max || data <= allowed[k]*multiplier[ct])
		  {
		    //cout<<ip<<" "<<t<<" "<<ct<<" normal \n";
		  }
		else
	    ...
	    if (tag > 0)
	      stats[ip].statsdata[time].data[t].tag[index] = tag;
	    else
	      {
		for (enum celltype ct = SRC; ct <= FLOW; ct=(enum celltype)((int)ct+ 1))
		  {
		    ...
		    if (metrics[ip][t].n == 1)
		      {
			metrics[ip][t].records[ct][index].mean =  data;
			metrics[ip][t].records[ct][index].ss = 0;
		      }
		    else
		      {
			double ao = metrics[ip][t].records[ct][index].mean;
			metrics[ip][t].records[ct][index].mean = metrics[ip][t].records[ct][index].mean +
			  (data - metrics[ip][t].records[ct][index].mean)/metrics[ip][t].n;
			metrics[ip][t].records[ct][index].ss = metrics[ip][t].records[ct][index].ss +
			  (data - ao)*(data - metrics[ip][t].records[ct][index].mean);
		      }
		    if (data > metrics[ip][t].records[ct][index].max)
		      {
			metrics[ip][t].records[ct][index].max = data;
		      }
		    //cout<<" normal\n";
		  }
	      }
  ...
  for(auto it = stats.begin(); it != stats.end(); it++)
      ...
      for (enum type t=TOTAL; t<=RPC; t=(enum type)((int)t + 1))
	{
	  ...
	  for (enum celltype ct = SRC; ct <= FLOW; ct=(enum celltype)((int)ct+ 1))
		...
		{
		  metrics[ip][t].records[ct][index].mean = 0;
		  metrics[ip][t].records[ct][index].ss = 0;
		  metrics[ip][t].records[ct][index].cusum = 0;
		}
	    }
			...
			for (int k = 0; k < 4; k++) // ALLOWED
			  {
			    ...
			    for (enum celltype ct = SRC; ct <= FLOW; ct=(enum celltype)((int)ct+ 1))
			      {
				double ao = metrics[ip][t].records[ct][index].mean;
				metrics[ip][t].records[ct][index].mean =  diff*metrics[ip][t].records[ct][index].mean/(diff+metrics[ip][t].n);
				metrics[ip][t].records[ct][index].ss = (metrics[ip][t].n*(metrics[ip][t].records[ct][index].ss+pow((ao-metrics[ip][t].records[ct][index].mean),2))+diff*pow(metrics[ip][t].records[ct][index].mean,2))/(diff+metrics[ip][t].n);		  
				metrics[ip][t].records[ct][index].cusum = 0;
			      }
			  }
