...
double read_one_line(void* nf, char* line)
{
  ...
  if (dl > 0)
      ...
      for(int i=1; i<dl-1; i+=4) // trailing comma
	{
	  ...
	  b.tag = 0;
	  cout<<"Type "<<t<<" srcs "<<b.srcs<<" vol "<<b.vol<<" flows "<<b.flows<<endl;
...
{
  cout<<" cusum srcs "<<value.srcs<<" vol "<<value.vol<<" flows "<<value.flows<<"\n";
...
void update_means(unsigned int ip, enum type t, struct bcell value, unsigned int time)  
{
  ...
  for (enum celltype ct = SRC; ct <= FLOW; ct=(enum celltype)((int)ct+ 1))
	...
	{
	  cout<<ip<<" type "<<t<<" ct "<<ct<<" normal , n is "<<metrics[ip][t].n <<" cusum "<<metrics[ip][t].records[ct].cusum<<" data "<<data<<"\n";
	}
      else
	{
	  cout<<" anomalous\n";
