...
map<int,int> tagmap;
map<unsigned int, set<unsigned int>> sources;
...
amonProcessing(flow_t flow, int len, double start, double end, int oci, int ooci, string file)
{
  ...
  if (flow.dlocal || flow.slocal)
	...
	{
	  attack a = *it;
	  if (a.target == flow.dst)
	    {
	      if((start <= a.end && start >= a.start) ||  (end < a.end && end > a.start))
	      {
...
void loadattacks(const char* fname)
{
  ifstream inFile;
  int i = 0;
  inFile.open(fname);
  ...
  while(true)
    {
      ...
      //Attack on 11.91.28.21 from 1589246412 to 1589250265 dur 1803 rate 65536 types 256
      inFile>>dummy;
      if (inFile.eof())
	break;
      attack a;
