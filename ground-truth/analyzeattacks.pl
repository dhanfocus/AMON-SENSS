# Compare actual alerts with the given alerts for different
...
for ($i = 0; $i < 3; $i++)
{
	...
	{
	    $id = $i*4*4+$j*4+$k;
	    print "$id i $i j $j k $k $results{$id}{'matched'} $results{$id}{'matchednum'}  $results{$id}{'missed'}   $results{$id}{'extras'} \n";
	}
    }
}
